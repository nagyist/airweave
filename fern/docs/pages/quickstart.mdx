...
response = httpx.post(
    f"https://api.airweave.ai/collections/{collection.readable_id}/search/instant",
    headers={"x-api-key": "YOUR_API_KEY"},
    json={"query": "Find returned payments from user John Doe?"},
)
...
const response = await fetch(
  `https://api.airweave.ai/collections/${collection.readable_id}/search/instant`,
  {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({ query: "Find returned payments from user John Doe?" }),
  }
);
