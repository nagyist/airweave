...
<Steps>
  ...
  <Step title="Configure search options (optional)">
    Adjust the retrieval strategy and max results to fine-tune your search.
  </Step>
  <Step title="Connect outputs to downstream blocks">
    Wire the block's `results` array output into downstream blocks such as an LLM agent, a Slack notification, or a condition filter.
  </Step>
</Steps>
...
| `retrievalStrategy` | string | `hybrid` | Retrieval strategy: `hybrid`, `semantic`, or `keyword` |
...
- **RAG-style question answering**: Connect the Airweave block to an LLM agent block. Pass the `results` output as context so the agent can answer questions grounded in your synced data.
