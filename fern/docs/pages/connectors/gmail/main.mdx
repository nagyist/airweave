...
|-------|------|-------------|
| thread_key | str | Stable Airweave thread key (thread_&lt;gmail_id&gt;) |
| gmail_thread_id | str | Native Gmail thread ID |
| title | str | Display title derived from snippet |
| last_message_at | Optional[datetime] | Timestamp of the most recent message in the thread |
...
| Field | Type | Description |
|-------|------|-------------|
| message_key | str | Stable Airweave message key (msg_&lt;gmail_id&gt;) |
| message_id | str | Native Gmail message ID |
| subject | str | Subject line (fallback applied if missing) |
| sent_at | datetime | Timestamp from the Date header (or internal date fallback) |
| internal_timestamp | datetime | Gmail internal timestamp representing last modification |
...
| web_url_value | Optional[str] | Direct Gmail URL for the message |
...
| Field | Type | Description |
|-------|------|-------------|
| attachment_key | str | Stable Airweave attachment key (attach_&lt;message&gt;_&lt;filename&gt;) |
...
| web_url_value | Optional[str] | URL to view the parent message in Gmail |
...
| thread_id | Optional[str] | Thread ID (optional if not provided by change record) |
