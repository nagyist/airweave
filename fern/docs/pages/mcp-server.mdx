...
| `GET` | `/.well-known/oauth-authorization-server` | OAuth discovery metadata (when OAuth enabled) |
| `POST` | `/register` | OAuth dynamic client registration (when OAuth enabled) |
| `GET` | `/authorize` | OAuth authorization redirect (when OAuth enabled) |
| `POST` | `/token` | OAuth token exchange (when OAuth enabled) |
| `POST` | `/revoke` | OAuth token revocation (when OAuth enabled) |
| `GET` | `/oauth/callback` | Auth0 callback handler (when OAuth enabled) |
...
- **Hosted mode**: Stateless HTTP server. A fresh MCP server instance is created for each request. Supports API key auth (via headers) and OAuth 2.0 (via Auth0). OAuth state (pending authorizations, registered clients) is stored in Redis.
...
**Hosted mode (Docker, API key only):**
...
docker run -p 8080:8080 \
  ...
  -e MCP_BASE_URL=https://mcp.your-domain.com \
...
| Variable | Required | Description |
|----------|----------|-------------|
| `AIRWEAVE_COLLECTION` | No | Default collection when `X-Collection-Readable-ID` header is absent |
| `AIRWEAVE_BASE_URL` | No | Airweave API URL (default: `https://api.airweave.ai`) |
| `MCP_OAUTH_ENABLED` | No | Set to `true` to enable OAuth 2.0 (default: `false`) |
| `MCP_BASE_URL` | OAuth | Public URL of the MCP server (used for OAuth metadata and redirects) |
| `AUTH0_DOMAIN` | OAuth | Your Auth0 tenant domain |
| `AUTH0_AUDIENCE` | OAuth | Auth0 API audience identifier |
| `AUTH0_CLIENT_ID` | OAuth | Auth0 application client ID |
| `AUTH0_CLIENT_SECRET` | OAuth | Auth0 application client secret |
| `REDIS_HOST` | OAuth | Redis host for OAuth state storage |
| `REDIS_PORT` | OAuth | Redis port (default: `6379`) |
| `REDIS_PASSWORD` | No | Redis password, if authentication is required |
| `MCP_REDIS_URL` | No | Full Redis URL (overrides `REDIS_HOST`/`REDIS_PORT`/`REDIS_PASSWORD`) |
