...
| `retrievalStrategy` | string | No | `"hybrid"`, `"semantic"`, or `"keyword"` (default: `"hybrid"`) |

See the [Search documentation](/search) for details on retrieval strategies.
...
2. **Action**: Airweave - Search Collection
...
export default defineComponent({
  ...
  async run({ steps, $ }) {
    const response = await axios($, {
      url: `${this.airweave.$auth.base_url || "https://api.airweave.ai"}/collections/my-collection/search/instant`,
      method: "POST",
      headers: {
        "x-api-key": this.airweave.$auth.api_key,
