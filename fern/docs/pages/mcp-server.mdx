...
| `search_method` | `"hybrid"` \| `"neural"` \| `"keyword"` | No | `"hybrid"` | Search strategy |

<Note>
The MCP server uses **legacy** parameter names (`search_method`, `response_type`, etc.) for backwards compatibility with existing AI assistant integrations. See the [Search documentation](/search) for the current API.
</Note>
