    ...
    async def increment_usage(
        ...
        *,
        ...
        if not current_usage:
            logger.warning(
                f"No current usage record for org {organization_id}. "
                f"Cannot increment usage without an active billing period."
            )
            return None
        ...
        if update_parts:
            ...
            if updated_row:
                ...
                # Create SQLAlchemy model first (without team_members or source_connections)
                updated_model = Usage(
                ...
                logger.warning("increment_usage: no row returned from UPDATE")
                return None
