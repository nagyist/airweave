...
_GET_DB_CTX = "airweave.domains.access_control.pipeline.get_db_context"
...
class TestApplyMembershipChanges:
    ...
    async def test_applies_adds_and_removes(self):
        ...
        assert adds == 2
        assert removes == 1
        assert pipeline._acl_repo.upsert.call_count == 2
        assert pipeline._acl_repo.delete_by_key.call_count == 1
...
class TestProcessIncremental:
    ...
    async def test_deleted_groups_remove_all_memberships(self):
        ...
        with patch(_GET_DB_CTX) as mock_db_ctx:
            ...
            pipeline._acl_repo.delete_by_group = AsyncMock(return_value=5)

            total = await pipeline._process_incremental(source, ctx, runtime)

        assert total == 10  # 5 members x 2 groups
        assert pipeline._acl_repo.delete_by_group.call_count == 2
    ...
    async def test_full_flow_with_adds_removes_and_deletes(self):
        ...
        with patch(_GET_DB_CTX) as mock_db_ctx:
            ...
            pipeline._acl_repo.upsert = AsyncMock()
            pipeline._acl_repo.delete_by_key = AsyncMock()
            pipeline._acl_repo.delete_by_group = AsyncMock(return_value=3)
        ...
        assert pipeline._acl_repo.upsert.call_count == 2
        assert pipeline._acl_repo.delete_by_key.call_count == 1
        assert pipeline._acl_repo.delete_by_group.call_count == 1
